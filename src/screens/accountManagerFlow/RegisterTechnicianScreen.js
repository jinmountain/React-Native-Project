...
const RegisterTechnicianScreen = ({ navigation }) => {
  ...
  const [ alertBoxText, setAlertBoxText ] = useState(null);
	...
	const [ techService, setTechService ] = useState([]); // nail for now 
            ...
            <View style={styles.pickServiceContainer}>
            {
            	services.map((item, index) => (
            		<Pressable
		              ...
		              style={{}}
		              onPress={() => {
		              	if (techService.includes(item.value)) {
		              		setTechService(techService.filter((service) => service !== item.value));
		              	} else {
		              		setTechService([ ...techService, item.value ]);
		              	}
		              }}
		            >
		            	<View style={styles.serviceOptionsContainer}>
		              	...
		              	{
		              		techService.includes(item.value) &&
